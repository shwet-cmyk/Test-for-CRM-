...
public class AuthService : IAuthService
{
    ...
    public async Task<LoginResultDto> LoginAsync(LoginRequestDto request)
    {
        ...
        {
            return new LoginResultDto { Success = false, ErrorMessage = "Username or email is required." };
        }
    ...
    public Task<IEnumerable<TenantSummaryDto>> GetTenantsForCurrentUserAsync()
    {
        return _identityGateway.GetTenantsForCurrentUserAsync();
    }

    public Task<LoginResultDto> SwitchTenantAsync(Guid tenantId)
    {
        return _identityGateway.SwitchTenantAsync(tenantId);
    }
}
